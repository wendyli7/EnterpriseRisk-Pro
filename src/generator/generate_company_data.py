...
BUSINESS_STATUSES = ["存续", "在业", "迁入", "开业", "经营异常", "注销"]
...
GIVEN_NAMES = ["伟", "芳", "娜", "敏", "静", "磊", "洋", "勇", "艳", "杰", "琳", "超"]
COMPANY_BRANDS = ["华辰", "启明", "远航", "中科", "联盛", "安信", "博远", "恒泰", "智联", "嘉禾"]
COMPANY_SUFFIXES = ["科技有限公司", "供应链有限公司", "制造有限公司", "信息技术有限公司"]
...
class DemoDataGenerator:
    ...
    def generate(self) -> GeneratedData:
        ...
        return GeneratedData(companies, financials, lawsuits, penalties, opinions)
            ...
            records.append(
                {
                    ...
                    "business_status": self.random.choices(
                        BUSINESS_STATUSES,
                        weights=[0.62, 0.20, 0.03, 0.08, 0.05, 0.02],
                        k=1,
                    )[0],
                }
            )
                ...
                records.append(
                    {
                        ...
                        "total_assets": assets,
                        "total_liabilities": self._money(float(assets) * ratio / 100),
                ...
                records.append(
                    {
                        ...
                        "penalty_date": self._random_date(1, 365 * 3).isoformat(),
                    }
                )
        return pd.DataFrame(records)
    ...
    def _generate_company_opinion(self, companies: pd.DataFrame) -> pd.DataFrame:
        records: list[dict[str, object]] = []
        for row in companies.itertuples(index=False):
            for index in range(self.random.randint(1, 4)):
                sentiment = self.random.choices(["positive", "neutral", "negative"], weights=[0.30, 0.40, 0.30], k=1)[0]
                title_pool = NEGATIVE_TITLES if sentiment == "negative" else POSITIVE_TITLES
                records.append(
                    {
                        "company_id": row.company_id,
                        "news_title": f"{row.company_name}{self.random.choice(title_pool)}",
                        "news_source": self.random.choice(NEWS_SOURCES),
                        "sentiment": sentiment,
                        "publish_date": self._random_date(1, 365 * 2).isoformat(),
    ...
    def _random_date(self, min_days_ago: int, max_days_ago: int) -> date:
        return date.today() - timedelta(days=self.random.randint(min_days_ago, max_days_ago))
